...
export default async function LoginPage({
  ...
  return (
    <main className="mx-auto grid min-h-[calc(100vh-73px)] max-w-7xl items-center px-5 py-10 sm:py-16">
      <section className="mobile-card mx-auto w-full max-w-md rounded-[2rem] card p-8">
        <p className="text-sm font-bold uppercase tracking-[0.18em] text-blue">Student Login</p>
        <h1 className="mobile-title mt-3 text-4xl font-semibold tracking-tight text-ink">Sign in to your courses</h1>
        ...
        <form action="/api/auth/login" method="post" className="mt-7 space-y-4">
          ...
          <button className="min-h-12 w-full rounded-full bg-blue px-5 py-3 text-sm font-bold text-white">Login</button>
        </form>
      </section>
    </main>
  );
